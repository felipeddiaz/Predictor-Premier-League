# -*- coding: utf-8 -*-
"""
optimizar_xgb_84feat.py
=======================
Optimiza XGBoost sobre las 84 features nuevas (incluyendo Pinnacle Move y Arbitro).
...
- Espacio de busqueda amplio (no centrado en params anteriores)
- Guarda JSON con mejores params en modelos/optuna_xgb_84feat.json
...
RUTA_JSON = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         '..', 'modelos', 'optuna_xgb_84feat.json')
...
def objetivo(trial, X_train, y_train, tscv):
    ...
    El gap entre el promedio CV y el fold mas reciente penaliza modelos
    que memorizan el pasado pero no generalizan al futuro.
    """
    ...
    # Penalizacion: gap > 1.5% penaliza fuerte (modelo sobreajustado al pasado)
...
def main(n_trials=150):
    print("=" * 65)
    print(f"  XGB OPTUNA — 84 features ({n_trials} trials)")
    ...
    # Baseline: params actuales con las 84 features
    ...
    print(f"  Baseline (params actuales, 84 feats): F1-test = {base_f1:.4f}")
    print()
    ...
    print(f"\n  F1 test={f1_test:.4f}  Acc={acc_test:.4f}  (vs baseline 84feat: {mejora:+.4f})")
    print()
