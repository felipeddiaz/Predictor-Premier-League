# -*- coding: utf-8 -*-
"""
fast_search2.py - Pre-cachea el DF procesado, luego busca params rapido
"""
import sys, os, time
...
CACHE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                     'datos', 'procesados', 'df_features_cache.pkl')
...
if os.path.exists(CACHE):
    ...
    print("Preparando datos (primera vez, ~20s)...")
    t0 = time.time()
    ...
    # Features rolling extra (vectorizado)
    df['_HT_gd'] = df['FTHG'] - df['FTAG']
    df['HT_Goals_Diff'] = df.groupby('HomeTeam')['_HT_gd'].transform(
        lambda x: x.shift(1).rolling(ROLLING, min_periods=1).mean()).fillna(0)
    ...
    df.to_pickle(CACHE)
    print(f"Datos preparados en {time.time()-t0:.1f}s — cache guardado")
...
param_grid = [
    ...
    {'n_estimators': 650, 'max_depth': 5, 'learning_rate': 0.018, 'subsample': 0.77,
     'colsample_bytree': 0.73, 'colsample_bylevel': 0.63, 'reg_alpha': 0.55,
     'reg_lambda': 2.7, 'min_child_weight': 5, 'gamma': 0.28},
    {'n_estimators': 420, 'max_depth': 6, 'learning_rate': 0.032, 'subsample': 0.84,
     'colsample_bytree': 0.79, 'colsample_bylevel': 0.74, 'reg_alpha': 0.28,
     'reg_lambda': 1.9, 'min_child_weight': 2, 'gamma': 0.12},
]
...
for r in resultados[:10]:
    print(f"  F1={r['f1']:.4f}  feat={r['feat']}  p={r['params_idx']}  w={r['pesos_idx']}  pesos={r['pesos']}")
