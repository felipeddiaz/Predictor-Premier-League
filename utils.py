...
def agregar_features_interaccion(df: pd.DataFrame) -> pd.DataFrame:
    """
    Agrega features de interaccion entre variables existentes.

    Estas features capturan relaciones que XGBoost necesita multiples splits
    para descubrir por si solo:

    1. Eficiencia xG: goles_reales / xG esperados (overperformance)
    2. Forma ponderada: puntos * dificultad del calendario
    3. Fatiga compuesta: congestion * competicion europea
    """
    print("\n\U0001f527 Calculando features de interaccion...")
    n_added = 0

    # --- 1. Eficiencia xG ---
    if 'HT_xG_Avg' in df.columns and 'HT_AvgGoals' in df.columns:
        # Evitar division por 0: clip xG a minimo 0.1
        ht_xg = df['HT_xG_Avg'].clip(lower=0.1)
        at_xg = df['AT_xG_Avg'].clip(lower=0.1)

        df['HT_xG_Efficiency'] = df['HT_AvgGoals'] / ht_xg
        df['AT_xG_Efficiency'] = df['AT_AvgGoals'] / at_xg
        df['xG_Efficiency_Diff'] = df['HT_xG_Efficiency'] - df['AT_xG_Efficiency']
        n_added += 3

    # --- 2. Forma ponderada por dificultad ---
    if 'HT_Pts5' in df.columns and 'HT_SoR5' in df.columns:
        # SoR normalizado para que el producto sea interpretable
        ht_sor = df['HT_SoR5'].clip(lower=0.01)
        at_sor = df['AT_SoR5'].clip(lower=0.01)

        df['HT_Form_vs_Elo'] = df['HT_Pts5'] * ht_sor
        df['AT_Form_vs_Elo'] = df['AT_Pts5'] * at_sor
        df['Form_Quality_Diff'] = df['HT_Form_vs_Elo'] - df['AT_Form_vs_Elo']
        n_added += 3

    # --- 3. Fatiga compuesta ---
    if 'HT_Games_15d' in df.columns and 'HT_Had_Europa' in df.columns:
        df['HT_Fatigue_Score'] = df['HT_Games_15d'] * (1 + df['HT_Had_Europa'])
        df['AT_Fatigue_Score'] = df['AT_Games_15d'] * (1 + df['AT_Had_Europa'])
        df['Fatigue_Diff'] = df['HT_Fatigue_Score'] - df['AT_Fatigue_Score']
        n_added += 3

    # Limpiar NaN en features nuevas
    new_cols = ['HT_xG_Efficiency', 'AT_xG_Efficiency', 'xG_Efficiency_Diff',
                'HT_Form_vs_Elo', 'AT_Form_vs_Elo', 'Form_Quality_Diff',
                'HT_Fatigue_Score', 'AT_Fatigue_Score', 'Fatigue_Diff']
    for col in new_cols:
        if col in df.columns:
            df[col] = df[col].fillna(0)

    print(f"   \u2705 Features de interaccion: {n_added} columnas agregadas")
    return df
