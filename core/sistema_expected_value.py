...
# ============================================================================
# DOBLE OPORTUNIDAD INTELIGENTE
# ============================================================================

def diagnostico_dc(prob_modelo_3: np.ndarray):
    """
    Analiza la distribucion de probabilidades del modelo y determina si
    la Doble Oportunidad es la apuesta natural (no un fallback).

    En vez de preguntar "que va a pasar?", pregunta "que NO va a pasar?".

    Retorna un dict con:
        - usar_dc (bool): si DC es recomendable
        - mercado (str): '1X', 'X2', o '12'
        - prob_dc (float): probabilidad combinada del DC
        - prob_descartada (float): probabilidad del resultado excluido
        - señal (str): cual señal lo activo
        - confianza_dc (str): 'alta', 'media', 'baja'
        - detalle (str): explicacion legible

    Señales:
        1. Anti-resultado: prob del peor resultado < PROB_DESCARTE_MAX
           El modelo descarta fuertemente un resultado.
        2. Concentracion: gap entre top2 < GAP_TOP2_MAX y
           gap al 3ro > GAP_TERCERO_MIN.
           Dos resultados son equiprobables, el tercero no.
        3. Ratio: prob_3ro / prob_2do < RATIO_DESCARTE_MAX.
           Forma robusta de detectar que un resultado es improbable.
    """
    if prob_modelo_3 is None or len(prob_modelo_3) != 3:
        return {'usar_dc': False, 'señal': 'sin_datos'}

    p = np.array([float(x) for x in prob_modelo_3])

    # Ordenar: idx[0]=mas probable, idx[2]=menos probable
    idx = np.argsort(p)[::-1]
    p_1ro, p_2do, p_3ro = p[idx[0]], p[idx[1]], p[idx[2]]

    # Mapeo de resultado excluido a mercado DC
    # idx_excluido -> mercado: excluir 0(Home)=X2, excluir 1(Draw)=12, excluir 2(Away)=1X
    _mercado_por_excluido = {0: 'X2', 1: '12', 2: '1X'}

    resultado = {
        'usar_dc': False,
        'mercado': None,
        'prob_dc': None,
        'prob_descartada': p_3ro,
        'idx_excluido': int(idx[2]),
        'señal': None,
        'confianza_dc': None,
        'detalle': None,
        'probs_ordenadas': (p_1ro, p_2do, p_3ro),
        'gap_top2': abs(p_1ro - p_2do),
        'ratio_descarte': p_3ro / p_2do if p_2do > 0 else 1.0,
    }

    # --- Señal 1: Anti-resultado ---
    # Si la prob del peor resultado es muy baja, DC es natural
    PROB_DESCARTE_MAX = 0.20
    if p_3ro < PROB_DESCARTE_MAX:
        resultado['usar_dc'] = True
        resultado['señal'] = 'anti_resultado'
        resultado['mercado'] = _mercado_por_excluido[idx[2]]
        resultado['prob_dc'] = p_1ro + p_2do
        resultado['detalle'] = (
            f"Modelo descarta resultado {idx[2]} (prob={p_3ro:.1%}). "
            f"DC {resultado['mercado']} cubre {resultado['prob_dc']:.1%}."
        )

    # --- Señal 2: Concentracion en dos resultados ---
    GAP_TOP2_MAX = 0.08
    GAP_TERCERO_MIN = 0.15
    gap_top2 = abs(p_1ro - p_2do)
    gap_tercero = p_2do - p_3ro

    if not resultado['usar_dc'] and gap_top2 < GAP_TOP2_MAX and gap_tercero > GAP_TERCERO_MIN:
        resultado['usar_dc'] = True
        resultado['señal'] = 'concentracion'
        resultado['mercado'] = _mercado_por_excluido[idx[2]]
        resultado['prob_dc'] = p_1ro + p_2do
        resultado['detalle'] = (
            f"Top 2 resultados son similares (gap={gap_top2:.1%}) "
            f"pero lejanos al 3ro (gap={gap_tercero:.1%}). "
            f"DC {resultado['mercado']} es la apuesta natural."
        )

    # --- Señal 3: Ratio de descarte ---
    RATIO_DESCARTE_MAX = 0.50
    ratio = p_3ro / p_2do if p_2do > 0 else 1.0

    if not resultado['usar_dc'] and ratio < RATIO_DESCARTE_MAX:
        resultado['usar_dc'] = True
        resultado['señal'] = 'ratio_descarte'
        resultado['mercado'] = _mercado_por_excluido[idx[2]]
        resultado['prob_dc'] = p_1ro + p_2do
        resultado['detalle'] = (
            f"Ratio 3ro/2do = {ratio:.2f} < {RATIO_DESCARTE_MAX}. "
            f"El resultado {idx[2]} es improbable relativo al 2do."
        )

    # Nivel de confianza del DC
    if resultado['usar_dc']:
        if p_3ro < 0.12:
            resultado['confianza_dc'] = 'alta'
        elif p_3ro < 0.18:
            resultado['confianza_dc'] = 'media'
        else:
            resultado['confianza_dc'] = 'baja'

    return resultado


def evaluar_dc_inteligente(
    prob_modelo_3: np.ndarray,
    cuota_h: float,
    cuota_d: float,
    cuota_a: float,
    umbral_edge: float = UMBRAL_EDGE_DC,
    solo_alta_media: bool = True,
):
    """
    Evalua DC como apuesta primaria usando diagnostico_dc().

    A diferencia de calcular_ev_doble_oportunidad() (que evalua todos los
    mercados DC por edge), esta funcion primero pregunta si la FORMA de la
    distribucion justifica DC, y solo despues verifica el edge.

    Args:
        prob_modelo_3: [prob_home, prob_draw, prob_away] del modelo
        cuota_h, cuota_d, cuota_a: cuotas 1X2 del mercado
        umbral_edge: edge minimo requerido
        solo_alta_media: si True, solo recomienda DC con confianza alta o media

    Returns:
        dict con mercado, edge, cuota, ev, diagnostico; o None si no aplica.
    """
    diag = diagnostico_dc(prob_modelo_3)

    if not diag['usar_dc']:
        return None

    if solo_alta_media and diag.get('confianza_dc') == 'baja':
        return None
    ...
    # Construir solo el mercado que el diagnostico recomienda
    mercado = diag['mercado']
    if mercado == '1X':
        p_model = p_h + p_d
        p_fair = fair_h + fair_d
    elif mercado == 'X2':
        p_model = p_d + p_a
        p_fair = fair_d + fair_a
    else:  # '12'
        p_model = p_h + p_a
        p_fair = fair_h + fair_a
